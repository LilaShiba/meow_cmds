...
class DataParser:
    ...
    def load_data(self):
        ...
        if data_frames:
            ...
            print(
                f"Loaded {len(data_frames)} files with {len(df_combined)} rows.")
            return df_combined
        else:
    ...
    def add_data(self, file_path):
        ...
        try:
            ...
            self.df = pd.concat(
                [self.df, new_data], ignore_index=True) if not self.df.empty else new_data
            print(
                f"Appended data from {file_path}. Total rows: {len(self.df)}.")
        except Exception as e:
            print(f"Error reading {file_path}: {e}")
    ...
    def plot_heatmap(self) -> None:
        ...
        trans_cmap = sns.color_palette(
            ["#55CDFC", "#F7A8B8", "#FFFFFF", "#F7A8B8", "#55CDFC"])

        plt.figure(figsize=(12, 8))
        sns.heatmap(self.df.iloc[:, 1:].corr(), cmap=trans_cmap,
    ...
    def linear_regression(self, target_column, feature_columns, verbose=True):
        ...
        missing_cols = [col for col in [target_column] +
                        feature_columns if col not in self.df.columns]
        if missing_cols:
